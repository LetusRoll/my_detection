...
void CVC::Process(const PtC::Ptr &in_cloud,vector<int> &cluster_indices)
{
...
//聚类
vector<int> CVC::Cluster(const PtC::Ptr &in_cloud,std::unordered_map<int,Voxel> &map,const vector<APR> &APR_cloud)
{
...
//寻找邻近体素的索引
void CVC::FindNeighbors(const PtC::Ptr &in_cloud,std::unordered_map<int,Voxel> &map,const int &theta_index,const int &rho_index,
...
Pt CVC::CalculateCenter(const PtC::Ptr &in_cloud,int self_index,std::unordered_map<int,Voxel> &map)
{
    Pt center;
